...
export default class Input extends Component {
	...
	studentCheck() {
		fetch(`${config.API_ENDPOINT}/`, {
			...
			if (!res.ok) alert('Student MARSS and/or Last Name Invalid');
		});
	}

	emailHandler(event) {
		this.setState({ submissionEmail: event.target.value });
	}

	submitterNameHandler(event) {
		this.setState({ submitterName: event.target.value });
	}

	studentFirstNameHandler(event) {
		this.setState({ studentFirstName: event.target.value });
	}

	studentLastNameHandler(event) {
		this.setState({ studentLastName: event.target.value });
	}

	studentGradeHandler(event) {
		this.setState({ grade: event.target.value });
	}

	iepManagerHandler(event) {
		this.setState({ iepManager: event.target.value });
	}

	involvedPeopleHandler(event) {
		this.setState({ involvedPeople: event.target.value });
	}

	directorHandler(event) {
		this.setState({ director: event.target.value });
	}

	marssHandler(event) {
		this.setState({ marss: event.target.value });
	}

	contributingVariablesHandler(event) {
		this.setState({ contributingVariables: event.target.value });
	}

	antecedentHandler(event) {
		this.setState({ antecedent: event.target.value });
	}

	disabilityHandler(event) {
		this.setState({ disability: event.target.value });
	}

	federalHandler(event) {
		this.setState({ federal: event.target.value });
	}
	...
	raceEthnicityHandler(event) {
		this.setState({ raceEthnicity: event.target.value });
	}
	...
	dateHandler(event) {
		this.setState({ dateHandler: event.target.value });
	}

	physicalHoldingHandler(event) {
		this.setState({ holds_used: event.target.value });
	}

	seclusionHandler(event) {
		this.setState({ seclusion: event.target.value });
	}

	forceHandler(event) {
		this.setState({ forceHandler: event.target.value });
	}
	...
	injuryHandler(event) {
		this.setState({ injury: event.target.value });
	}

	lawEnforcmentHandler(event) {
		this.setState({ lawEnforcment: event.target.value });
	}
	...
	roomlocationHandler(event) {
		this.setState({ roomlocation: event.target.value });
	}

	disruptionHandler(event) {
		this.setState({ disruption: event.target.value });
	}
	...
	handleSubmit(event) {
		...
		//this.studentCheck();
		if (
			this.state.major_disruption === '---' ||
			this.state.roomlocation === '---' ||
			this.state.day_of_the_week === '---' ||
			this.state.holds === '---'
		) {
				...
				<div>
					...
					<nav>
						<label htmlFor="school">
							At which site did the Incident occur?
						</label>
						<select
							id="school"
							name="schoolLocation"
