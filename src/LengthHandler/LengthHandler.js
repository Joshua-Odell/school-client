...
export default class LengthHandler extends Component {
	...
	lengthHandler = () => {
		...
		let time = '';
		if (start > stop) {
			this.context.stateGeneralUpdate(
				'holdError',
		...
		this.context.stateGeneralUpdate('length', time, () => {
			this.addHold();
		});
	};
	...
	addHold = () => {
		...
		if (
			this.context.reasonable_force === 'Non-PCM Hold' ||
			this.context.reasonable_force === 'Unlicensed Seclusion'
		) {
		...
		if (this.context.count > 5) {
			this.context.setState(
				'holdError',
				'There is a maximum of five holds allowed'
			);
		}
	...
	settingHoldIds = (id) => {
		...
		if (
			...
			this.context.enteredHoldList.push(
				' ' + this.context.reasonable_force + ','
			);
